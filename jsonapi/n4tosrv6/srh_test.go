...
func ExampleParseSRH_single() {
	srh, err := n4tosrv6.ParseSRH("fd00::1")
	if err != nil {
		panic(err)
	}
	fmt.Println(srh)
...
func ExampleParseSRH_multi() {
	srh, err := n4tosrv6.ParseSRH("fd00::1", "fd00::2")
	if err != nil {
		panic(err)
	}
	fmt.Println(srh)
...
func ExampleParseSRH_fromSlice() {
	slice := []string{"fd00::1", "fd00::2"}
	// …
	if len(slice) == 0 {
		panic("SRH must contains at least one segment")
	}
	srh, err := n4tosrv6.ParseSRH(slice[0], slice[1:]...)
	if err != nil {
		panic(err)
	}
	fmt.Println(srh)
...
func TestNewSRH(t *testing.T) {
	if srh, err := n4tosrv6.ParseSRH("::", "fd00::"); err == nil {
		if len(srh) != 2 {
